...
const Default = ({ getData = [] }) => {
  ...
  function getDataFc() {
    axios.get(`${API_URL}/returnStatus/${id}`).then((response) => {
  ...
  const onSubmit = async (Data) => {
    ...
    axios
      .post(`${API_URL}/returnStatus/${id}`, Data)
      .then((res) => {
  ...
  return (
    <div>
      <Card
        className="card"
        title={"Edit Return Order Status"}
      >
        <Form
          {...formItemLayout}
          form={form}
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const getData = await axios.get(API_URL + "/returnStatus/" + query.id, {
