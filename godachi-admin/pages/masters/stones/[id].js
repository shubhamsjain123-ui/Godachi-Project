...
const Default = () => {
  ...
  function getData() {
    axios.get(`${API_URL}/masters/stones/${id}`).then((response) => {
  ...
  const onSubmit = (Data) => {
    axios
      .post(`${API_URL}/masters/stones/${id}`, Data)
      .then((res) => {
        if (res.data.master == "error") {
          message.error(
            "Stone Not Updated" + res.data.messagge
          );
        } else {
          message.success("Stone Updated");
  ...
  return (
    <div>
      <Card className="card" title="Edit Stone">
        <Form
          {...formItemLayout}
          form={form}
          name="add"
