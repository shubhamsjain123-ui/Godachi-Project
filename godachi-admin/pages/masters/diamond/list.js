...
const Default = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
		...
		{role["masters/id"] ? (
            <Link href={"/masters/diamond/" + record._id}>
              <a>
                {" "}
                <EditOutlined
  ...
  const getData = () => {
    axios
      .get(API_URL + `/masters/diamondVariants`)
      .then((response) => {
  ...
  const deleteData = (id) => {
    axios.delete(`${API_URL}/masters/diamondVariants/${id}`).then(() => {
