...
const Default = () => {
  ...
  function getData() {
    axios.get(`${API_URL}/masters/promises/${id}`).then((response) => {
  ...
  const onSubmit = async (Data) => {
    ...
    if (Data.image != undefined && state.image != Data.image) {
      axios.post(`${API_URL}/upload/deletePromiseImage`, {
        path: state.image,
      });
    ...
    axios
      .post(`${API_URL}/masters/promises/${id}`, Data)
      .then((res) => {
        if (res.data.master == "error") {
          message.error(
            "Promise Not Updated" + res.data.messagge
          );
        } else {
          message.success("Promise Updated");
  ...
  return (
    <div>
      <Card className="card" title="Edit Promise">
        <Form
          {...formItemLayout}
          form={form}
          name="add"
