...
const Default = () => {
  ...
  function getData() {
    axios.get(`${API_URL}/masters/occassions/${id}`).then((response) => {
  ...
  const onSubmit = async (Data) => {
    ...
    axios
      .post(`${API_URL}/masters/occassions/${id}`, Data)
      .then((res) => {
        if (res.data.master == "error") {
          message.error(
            "Occassion Not Updated" + res.data.messagge
          );
        } else {
          message.success("Occassion Updated");
  ...
  return (
    <div>
      <Card className="card" title="Edit Occassion">
        <Form
          {...formItemLayout}
          form={form}
          name="add"
