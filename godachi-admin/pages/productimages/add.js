      ...
      .then((res) => {
        if (res.data.length > 0) {
          ...
          for (const i in res.data) {
            dataManipulate.push({
              label: res.data[i].variant_products[0].productCode + " - " + res.data[i].productName,
  ...
  const onSubmit = async (Data) => {
    Data.arrayImage.map(async (value) => {
      ...
      if (value.image != undefined) {
        ...
        value["image"] = dataImage.data.path.replace(/\\/g, '/').replace("public/", "/");
        value["mimeType"]= dataImage.data.mimeType;
        console.log(value);
        axios
      ...
      >
        ...
        >
          ...
          <div style={{textAlign: "center", paddingBottom:"10px"}}>
            ...
            <Text type="danger">Maximum Size: 2 MB</Text><br/>
            <Text type="danger">Recomended Size: 600 X 600</Text><br/>
          </div>
                    ...
                    >
                      <Upload
                        ...
                        beforeUpload={(file) => {
                          ...
                          if (!isJPG && !isVideo) {
                            message.error(
                              "Only Image and mp4 are allowed"
                            );
